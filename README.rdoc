...
* RubyGems
* Jabber::Simple 0.8.7+
* FFI, libotr and ffi-otr (optional; only for encryption support)
...
To use OTR encryption, make sure you have the otr library, FFI and ffi-otr installed.
Then just configure a file to store the privkey and fingerprints, optionally a policy,
and everything else will happen automatically.

Example OTR configuration:
  ...
  config = {
    ...
    :master    => 'master@example.com',
    :otr       => {
      :privkey => "data/privkey.otr",
      :fingerprints => "data/fingerprints.otr",
    }
  }

TODO: integrate OTR dependencies into gemspec
